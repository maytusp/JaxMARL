...
AD_HOC_METHODS = (
    "ph2v5_ad_hoc_teamplay",
    "ph2v5_ablate_ad_hoc_teamplay",
    "ph2v4_ad_hoc_teamplay",
    ...
    # "lmpred_gamma0_ad_hoc_teamplay",
    # "lmpred_gamma0_ablate_ad_hoc_teamplay",
    # "lmpred_gamma09_ad_hoc_teamplay",
    # "lmpred_gamma09_ablate_ad_hoc_teamplay",
    "lmpred_no_self_pred_ad_hoc_teamplay",
    "lmpred_ablate_no_self_pred_ad_hoc_teamplay",
...
def main() -> None:
    ...
    zsc_ps_np_gain_rows = collect_ad_hoc_rows(
        zsc_root,
        zsc_methods,
        layouts,
        "alignment_summary.csv",
        "average_alignment_ps_np_gain",
        "standard_error_alignment_ps_np_gain",
        4,
    )
    zsc_np_rows = collect_ad_hoc_rows(
        zsc_root,
        zsc_methods,
        layouts,
        "alignment_summary.csv",
        "average_alignment_neural_predictivity",
        "standard_error_alignment_neural_predictivity",
        4,
    )
    zsc_blind_np_rows = collect_ad_hoc_rows(
        zsc_root,
        zsc_methods,
        layouts,
        "alignment_summary.csv",
        "average_alignment_blind_neural_predictivity",
        "standard_error_alignment_blind_neural_predictivity",
        4,
    )
    ...
    ad_hoc_ps_np_gain_rows = collect_ad_hoc_rows(
        ad_hoc_root,
        ad_hoc_methods,
        layouts,
        "alignment_summary.csv",
    ...
    ad_hoc_np_rows = collect_ad_hoc_rows(
        ad_hoc_root,
        ad_hoc_methods,
        layouts,
        "alignment_summary.csv",
    ...
    ad_hoc_blind_np_rows = collect_ad_hoc_rows(
        ad_hoc_root,
        ad_hoc_methods,
        layouts,
        "alignment_summary.csv",
    ...
    if zsc_ps_np_gain_rows:
        write_csv(output_dir / "zsc_ps_np_gain.csv", alignment_headers, zsc_ps_np_gain_rows)
        sections.append(
            ("ZSC PS-NP Gain", markdown_table(alignment_headers, zsc_ps_np_gain_rows))
        )
    if zsc_np_rows:
        write_csv(output_dir / "zsc_neural_predictivity.csv", alignment_headers, zsc_np_rows)
        sections.append(
            ("ZSC Neural Predictivity", markdown_table(alignment_headers, zsc_np_rows))
        )
    if zsc_blind_np_rows:
        write_csv(
            output_dir / "zsc_blind_neural_predictivity.csv",
            alignment_headers,
            zsc_blind_np_rows,
        )
        sections.append(
            (
                ...
                markdown_table(alignment_headers, zsc_blind_np_rows),
            )
        )
    if ad_hoc_ps_np_gain_rows:
        write_csv(output_dir / "ad_hoc_ps_np_gain.csv", ad_hoc_headers, ad_hoc_ps_np_gain_rows)
        sections.append(
            (
                "Ad-Hoc Teamplay PS-NP Gain",
                markdown_table(ad_hoc_headers, ad_hoc_ps_np_gain_rows),
            )
        )
    if ad_hoc_np_rows:
        write_csv(output_dir / "ad_hoc_neural_predictivity.csv", ad_hoc_headers, ad_hoc_np_rows)
        sections.append(
            (
                "Ad-Hoc Teamplay Neural Predictivity",
                markdown_table(ad_hoc_headers, ad_hoc_np_rows),
            )
        )
    if ad_hoc_blind_np_rows:
        write_csv(
            output_dir / "ad_hoc_blind_neural_predictivity.csv",
            ad_hoc_headers,
            ad_hoc_blind_np_rows,
        )
        sections.append(
            (
                "Ad-Hoc Teamplay Blind-Control Neural Predictivity",
                markdown_table(ad_hoc_headers, ad_hoc_blind_np_rows),
            )
        )
