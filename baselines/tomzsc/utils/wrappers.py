...
class CTRolloutManager(JaxMARLWrapper):
    ...
    def __init__(self, env: MultiAgentEnv, batch_size:int, training_agents:List=None, preprocess_obs:bool=True):
        ...
        get_obs = (lambda x:x.flatten()) if preprocess_obs else (lambda x:x)
        self.global_state = lambda obs, state:  jnp.concatenate([get_obs(obs[agent]) for agent in self.agents], axis=-1)
    ...
    @partial(jax.jit, static_argnums=0)
    def batch_step(self, key, states, actions,**kwargs):
        keys = jax.random.split(key, self.batch_size)
        return jax.vmap(self.wrapped_step, in_axes=(0, 0, 0))(keys, states, actions,**kwargs)
    ...
    @partial(jax.jit, static_argnums=0)
    def wrapped_step(self, key, state, actions,**kwargs):
        obs_, state, reward, done, infos = self._env.step(key, state, actions,**kwargs)
        if self.preprocess_obs:
            obs = jax.tree_util.tree_map(self._preprocess_obs, {agent:obs_[agent] for agent in self.agents}, self.agents_one_hot)
    ...
    def get_valid_actions(self, state):
        ...
        valid = {agent:jnp.tile(actions, self.batch_size).reshape(self.batch_size, -1) for agent, actions in self.valid_actions_oh.items()}
        if hasattr(self._env,"get_valid_actions"):
            mask = jax.vmap(self._env.get_valid_actions)(state) # vmap over batch dim 
            valid = jax.tree.map(lambda x,y:x.at[:,:y.shape[1]].mul(y),valid,mask)
        return valid
