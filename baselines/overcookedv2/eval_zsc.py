...
def default_result_name(config: Dict) -> str:
    ...
    return str(config.get("TRAINING_METHOD", "sp"))
...
def make_xp_evaluator(config: Dict, method_module, pool: Dict):
    eval_config = dict(config)
    ...
    def evaluator(rng):
        xp_matrix = np.zeros((num_agents, num_agents), dtype=np.float32)
...
def main(config):
    ...
    evaluator = make_xp_evaluator(config, method_module, pool)
