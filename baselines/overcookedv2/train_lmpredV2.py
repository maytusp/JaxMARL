'''
Build on train_lmpred.py
Use target hidden state produced by an RNN from EMA parameters instead of current parameters.
'''
import jax
...
def _copy_self_stream_to_other_stream(two_stream_params):
    params = unfreeze(two_stream_params)
    target = params["params"] if "params" in params else params

    target["other_cnn"] = target["self_cnn"]
    target["other_ln"] = target["self_ln"]
...
def _update_ema_params(ema_params, params, decay):
...
def make_train(config):
    ...
    def train(rng, seed_idx):
        ...
        network_params = _copy_self_stream_to_other_stream(network_params)
        ema_params = network_params
        target_ema_decay = config.get(
            "RNN_TARGET_EMA_DECAY",
            config.get("EMA_DECAY", 0.996),
        )
        ...
        def _update_step(runner_state, unused):
            ...
            # CALCULATE ADVANTAGE
            train_state, ema_params, env_state, last_obs, last_done, update_step, ego_hstate, rng = (
                runner_state
            )
            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(carry, batch_info):
                    ...
                    def _loss_fn(
                        ...
                        _, _, _, target_aux = ego_network.apply(
                            ema_params,
                            jax.tree_util.tree_map(lambda h: h.squeeze(), init_hstate),
                            (traj_batch.obs, traj_batch.done),
                        )
                        ...
                        target_hidden = target_aux["rnn_hidden"]
                        target_pred_hidden_repr = target_aux["pred_hidden_repr"]
                        next_hidden = jnp.concatenate(
                            [
                                target_hidden[1:],
                                jnp.zeros_like(target_hidden[-1:]),
                            ],
                            axis=0,
                        )
                        next_pred_hidden_repr = jnp.concatenate(
                            [
                                target_pred_hidden_repr[1:],
                                jnp.zeros_like(target_pred_hidden_repr[-1:]),
                            ],
                            axis=0,
                        )
                        ...
                        target_ema_decay_metric = jnp.asarray(
                            target_ema_decay, dtype=pred_loss.dtype
                        )
                        ...
                        loss_metrics = {
                            ...
                            "ratio_max": ratio.max(),
                            "rnn_target_ema_decay": target_ema_decay_metric,
                        }
                    ...
                    ema_source_params = _copy_self_stream_to_other_stream(
                        train_state.params
                    )
                    ...
                    return (train_state, ema_params), loss_metrics
            ...
            train_state = train_state.replace(
                params=_copy_self_stream_to_other_stream(train_state.params)
            )
...
def main(config):
    ...
    elif np.isclose(self_pred_coef, 0.2):
        model_name = "lmpred02"
    elif np.isclose(self_pred_coef, 0.4):
        model_name = "lmpred04"
    else:
        model_name = "lmpred"
    self_pred_gammas = config.get("SELF_PRED_GAMMAS", (0.0, 0.5, 0.9))
    if isinstance(self_pred_gammas, str):
        gamma0_only = self_pred_gammas.replace(" ", "") == "[0.0]"
    else:
        gamma0_only = len(self_pred_gammas) == 1 and np.isclose(
            float(self_pred_gammas[0]), 0.0
        )
    if gamma0_only:
        model_name += "_gamma0"
