...
hidden_state --> Hidden_Predictor --> pred_next_hidden
target_hidden = stop_grad(next_hidden_state)
'''
import jax
...
class TwoStreamActorCriticRNN(nn.Module):
    ...
    def __call__(self, hidden, x):
        ...
        pred_next_hidden = nn.Dense(
            self.config["FC_DIM_SIZE"],
            kernel_init=orthogonal(jnp.sqrt(2)),
            bias_init=constant(0.0),
            name="self_pred_fc",
        )(embedding)
        pred_next_hidden = activation(pred_next_hidden)
        pred_next_hidden = nn.Dense(
            embedding.shape[-1],
            kernel_init=orthogonal(1.0),
            bias_init=constant(0.0),
        ...
        aux = {
            ...
            "pred_next_hidden": pred_next_hidden,
        }
            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(train_state, batch_info):
                    ...
                    def _loss_fn(params, init_hstate, traj_batch, gae, targets):
                        ...
                        pred_next_hidden = aux["pred_next_hidden"]
                        ...
                        pred_mask = (not_done * has_next_step)[..., None]
                        pred_target = jax.lax.stop_gradient(next_hidden)
                        pred_error_clip = config.get("SELF_PRED_ERROR_CLIP", 10.0)
                        pred_error = jnp.clip(
                            pred_next_hidden - pred_target,
                            -pred_error_clip,
                            pred_error_clip,
                        )
                        ...
                        pred_loss = (pred_loss * pred_mask).sum() / jnp.maximum(
                            pred_mask.sum() * pred_loss.shape[-1], 1.0
                        )
