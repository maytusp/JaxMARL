...
class ActorCriticRNN(nn.Module):
    ...
    def __call__(self, hidden, x):
        ...
        # Keep the auxiliary SF branch in this loader-only single-agent network
        # so train_single_sf.py checkpoints deserialize with the same param tree.
        sf_pred = nn.Dense(
...
def _copy_single_trunk_to_two_stream(two_stream_params, single_params):
    ...
    embedding_ln_name = "LayerNorm_1" if "LayerNorm_1" in single else "LayerNorm_0"
    target["self_ln"] = single[embedding_ln_name]
    target["other_ln"] = single[embedding_ln_name]
