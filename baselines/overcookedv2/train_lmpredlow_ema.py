'''
Build on ph2v5
No phase 1, initialise both streams from scratch.
Predict RNN input (fusion) instead of hidden state (like predictive coding)
'''
import jax
...
class TwoStreamActorCriticRNN(nn.Module):
    ...
    def __call__(self, hidden, x):
        ...
        ) and not self.config.get("USE_OTHER_STREAM_EMA", True)
...
def _copy_self_cnn_to_other_cnn(two_stream_params):
...
def _ema_other_cnn_from_self_cnn(two_stream_params, decay):
    params = unfreeze(two_stream_params)
    target = params["params"] if "params" in params else params

    target["other_cnn"] = jax.tree_util.tree_map(
        lambda other, self_: decay * other + (1.0 - decay) * self_,
        target["other_cnn"],
        target["self_cnn"],
    )
...
def _cosine_increase_schedule(init_value, end_value, transition_steps):
    def schedule(count):
        progress = jnp.minimum(count / transition_steps, 1.0)
        scale = 0.5 * (1.0 - jnp.cos(jnp.pi * progress))
        return init_value + (end_value - init_value) * scale
...
def _build_trainable_labels(params, config):
    ...
    finetune_other_stream = config.get(
        "FINETUNE_OTHER_STREAM", False
    ) and not config.get("USE_OTHER_STREAM_EMA", True)
...
def make_train(config):
    ...
    def train(rng, seed_idx):
        ...
        network_params = _copy_self_cnn_to_other_cnn(network_params)

        ema_updates = (
            config["NUM_UPDATES"] * config["UPDATE_EPOCHS"] * config["NUM_MINIBATCHES"]
        )
        other_stream_ema_decay_fn = _cosine_increase_schedule(
            config.get("OTHER_STREAM_EMA_DECAY", 0.996),
            config.get("OTHER_STREAM_EMA_DECAY_END", 1.0),
            max(ema_updates - 1, 1),
        )
            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(train_state, batch_info):
                    ...
                    def _loss_fn(params, init_hstate, traj_batch, gae, targets):
                        ...
                        rnn_input = aux["rnn_input"]
                        pred_hidden_repr = aux["pred_hidden_repr"]
                        next_rnn_input = jnp.concatenate(
                            [rnn_input[1:], jnp.zeros_like(rnn_input[-1:])], axis=0
                        )
                    ...
                    ema_step = train_state.step
                    train_state = train_state.apply_gradients(grads=grads)
                    if config.get("USE_OTHER_STREAM_EMA", True):
                        other_stream_ema_decay = other_stream_ema_decay_fn(
                            ema_step
                        )
                        train_state = train_state.replace(
                            params=_ema_other_cnn_from_self_cnn(
                                train_state.params,
                                other_stream_ema_decay,
                            )
                        )
                        loss_metrics["other_stream_ema_decay"] = other_stream_ema_decay
