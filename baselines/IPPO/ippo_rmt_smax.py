...
"""
# CUDA_VISIBLE_DEVICES=1 python -m baselines.IPPO.ippo_rmt_smax
import jax
...
def make_train(config):
    ...
    dummy_params = dummy_network.init(dummy_rng, dummy_memory, dummy_obs)
...
@hydra.main(version_base=None, config_path="config/smax", config_name="ippo_rmt_3s5z_vs_3s6z")
def main(config):
    ...
    wandb.init(
        ...
        tags=["IPPO", "RMT"],
        config=config,
        name=config["RUN_NAME"] + "_" + str(config["SEED"]),
