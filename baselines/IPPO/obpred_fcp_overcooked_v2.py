...
class ActorCriticPerspectiveAuxRNN(nn.Module):
    action_dim: Sequence[int]
...
def make_partner_obs_batch(obs_dict, agent_list):
    obs_agents = jnp.stack([obs_dict[a] for a in agent_list])
    assert len(agent_list) == 2, "Perspective auxiliary target currently assumes 2 agents."
    partner_obs_agents = obs_agents[::-1]
    return partner_obs_agents.reshape((-1,) + obs_agents.shape[2:])



def build_ego_network(env, config):
    network_ego = ActorCriticPerspectiveAuxRNN(
...
def make_train(config, eval_pop_params):
    ...
    env = OvercookedV2LogWrapper(env, replace_info=False)
    obs_shape = env.observation_space().shape
    perspective_coef = config.get("PERSPECTIVE_AUX_COEF", 1.0)
    ...
    def train(rng, pop_params):
        ...
        def _update_step(runner_state, unused):
            def _env_step(runner_state, unused):
                ...
                ac_in_partner = (obs_partner[jnp.newaxis, :], last_done[jnp.newaxis, :])
                ...
                def _update_minbatch(train_state, batch_info):
                    ...
                    def _loss_fn(params, init_hstate, traj_batch, gae, targets):
                        ...
                        total_loss = (
                            ...
                            + perspective_coef * perspective_loss
                        )
                        aux = LossInfo(
            ...
            metric["loss/perspective_aux"] = loss_info.perspective_loss.mean()
...
@hydra.main(version_base=None, config_path="config/oc_extended", config_name="fcp_overcooked_v2")
def main(config):
    ...
    model_name = "perspective_aux_fcp"

    wandb.init(
        entity=config["ENTITY"],
        project=config["PROJECT"],
        tags=["IPPO", "RNN", "OvercookedV2", "PerspectiveAux", "FCP"],
        config=config,
    ...
    train_pop_params = load_partner_pool(config, config["FCP_TRAIN_CHECKPOINTS"], dummy_params)
    eval_pop_params = load_partner_pool(config, config["FCP_EVAL_CHECKPOINTS"], dummy_params)
