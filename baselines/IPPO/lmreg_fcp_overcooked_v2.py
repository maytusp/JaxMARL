...
class ActorCriticToMRNN(nn.Module):
    ...
    def __call__(self, hidden, x):
        ...
        # Unpack 3 hidden states for separate mode: Self, Other (RL), Other (Mimic)
        hidden_self, hidden_other, hidden_other_mimic = hidden
        ...
        hidden_self, rnn_emb_self = rnn_self(hidden_self, (emb_self, dones))

        # --- 2. Other Forward Pass (Mimic for Like-Me Regularization) ---
        # Feeds self observation to the other stream to mimic ego representations
        emb_other_mimic = jax.vmap(embed_model_other)(obs_self)
        emb_other_mimic = layernorm_other(emb_other_mimic)
        hidden_other_mimic, rnn_emb_other_mimic = rnn_other(hidden_other_mimic, (emb_other_mimic, dones))

        # --- 3. Other Forward Pass (RL / ToM Prediction) ---
        # Feeds perspective transformed observations to other stream for RL policy
        if self.config.get("PERSPECTIVE_TRANSFORM", True):
            emb_other_rl = jax.vmap(embed_model_other)(obs_other)
        else:
            emb_other_rl = jax.vmap(embed_model_other)(obs_self)
            
        emb_other_rl = layernorm_other(emb_other_rl)
        hidden_other, rnn_emb_other_rl = rnn_other(hidden_other, (emb_other_rl, dones))

        if self.config.get("STOP_GRAD_OTHER", False):
            rnn_emb_other_rl = jax.lax.stop_gradient(rnn_emb_other_rl)
            hidden_other = jax.lax.stop_gradient(hidden_other)

        # Store representations to auxiliary dictionary for custom losses
        aux_info["emb_self_sg"] = jax.lax.stop_gradient(rnn_emb_self)
        aux_info["emb_other_mimic"] = rnn_emb_other_mimic

        # Partner action prediction loss MLP
        partner_action_mean = nn.Dense(
            self.config["FC_DIM_SIZE"],
            kernel_init=orthogonal(2),
            bias_init=constant(0.0),
        )(rnn_emb_other_rl)
        partner_action_mean = nn.relu(partner_action_mean)
        ...
        aux_info["partner_action_logits"] = partner_action_logits

        combined_embedding = jnp.concatenate([rnn_emb_self, rnn_emb_other_rl], axis=-1)
        new_hidden = (hidden_self, hidden_other, hidden_other_mimic)
        ...
        return new_hidden, pi, jnp.squeeze(critic, axis=-1), aux_info
...
class Transition(NamedTuple):
    ...
    obs: jnp.ndarray
    info: jnp.ndarray
    action_partner: jnp.ndarray
    partner_present: jnp.ndarray
    ...
    def evaluate_one_partner(rng, ego_params, partner_params):
        def run_one_episode(rng):
            ...
            def _step_fn(carry, _):
                ...
                hstate_partner, pi_partner, _, _ = network_partner.apply(
    ...
    def train(rng, pop_params):
        ...
        def _update_step(runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                ...
                # Check partner presence using channel 9 (agent_features_len index)
                # Assumes channels 9-17 hold partner information in the ego's raw obs
                partner_present = jnp.max(obs_ego[..., 9], axis=(1, 2))
            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(train_state, batch_info):
                    ...
                    def _loss_fn(params, init_hstate, traj_batch, gae, targets):
                        ...
                        _, pi, value, aux_info = network_ego.apply(
                            params,
                            squeezed_hstate,
                        ...
                        # Apply new aux losses for ToM model
                        if "emb_other_mimic" in aux_info and "emb_self_sg" in aux_info:
                            # 1. Like-me regularization (Mimic Pass vs Self Pass)
                            like_me_loss = jnp.mean(jnp.square(aux_info["emb_other_mimic"] - aux_info["emb_self_sg"]))
                            
                            # 2. Partner action prediction
                            logits = aux_info["partner_action_logits"]
                            labels = traj_batch.action_partner
                            
                            # Calculate CE loss
                            labels_one_hot = jax.nn.one_hot(labels, logits.shape[-1])
                            ce_loss = optax.softmax_cross_entropy(logits, labels_one_hot)
                            
                            # Mask out unobserved situations
                            mask = traj_batch.partner_present
                            masked_ce_loss = jnp.sum(ce_loss * mask) / (jnp.sum(mask) + 1e-8)
                            
                            total_loss += config.get("LIKE_ME_COEF", 0.1) * like_me_loss
                            total_loss += config.get("PARTNER_PRED_COEF", 1.0) * masked_ce_loss
                            
                            loss_aux = (value_loss, loss_actor, entropy, like_me_loss, masked_ce_loss)
                        else:
                            loss_aux = (value_loss, loss_actor, entropy, 0.0, 0.0)

                        return total_loss, loss_aux
